...
UTEST(allocate_page, tiny) {
  ...
  ASSERT_GE(page->free->header.size, (TINY_ZONE_SIZE + CHUNK_HEADER_SIZE) * NB_ALLOCATION_ZONE);
...
UTEST(allocate_page, small) {
  ...
  ASSERT_TRUE(page->next == NULL);
  ASSERT_TRUE(page->previous == NULL);
  ASSERT_GE(page->free->header.size, (SMALL_ZONE_SIZE + CHUNK_HEADER_SIZE) * NB_ALLOCATION_ZONE);
  ASSERT_EQ(munmap(page, page->size), 0);
}
...
UTEST(get_memory_allocation, tiny) {
    ...
    for (uint32_t i = 0; i < NB_ALLOCATION_ZONE; i++) {
        ...
        ASSERT_GE(zone->free->header.size, (TINY_ZONE_SIZE + CHUNK_HEADER_SIZE) * (NB_ALLOCATION_ZONE - 1 - i));
...
UTEST(get_memory_allocation, small) {
    ...
    for (uint32_t i = 0; i < NB_ALLOCATION_ZONE; i++) {
        ...
        ASSERT_GE(zone->free->header.size, (SMALL_ZONE_SIZE + CHUNK_HEADER_SIZE) * (NB_ALLOCATION_ZONE - 1 - i));
        ASSERT_EQ(zone->next, NULL);
        ASSERT_EQ(zone->previous, NULL);
    }
    ASSERT_EQ(munmap(zone, zone->size), 0);
...
UTEST(get_memory_allocation, large) {
